...
function splitTextIntoChunks(text, maxTokens = CHUNK_CONFIG.maxTokens, overlapTokens = CHUNK_CONFIG.overlapTokens) {
  ...
  while (startIndex < cleanText.length) {
    ...
    if (endIndex >= cleanText.length) {
      ...
      // Buscar un punto de corte natural (fin de oración o párrafo)
      ...
      for (const bp of breakPoints) {
        const matches = [...searchText.matchAll(new RegExp(bp.pattern, 'g'))];
        if (matches.length > 0) {
          // Tomar el último match cerca del límite
