...
const { User, Conversation, Message, Feedback, Blocklist } = require('../models');
...
router.patch('/users/:id', async (req, res) => {
  try {
    ...
    // Si se está bloqueando al usuario, añadir a la blocklist
    if (isActive === false) {
      await Blocklist.blockUser(
        user.email,
        user.lastIp,
        user.id,
        req.userId,
      ...
      console.log(`Usuario bloqueado: ${user.email}, IP: ${user.lastIp}`);
    }

    // Si se está desbloqueando, quitar de la blocklist
    if (isActive === true) {
      await Blocklist.unblockUser(user.email, user.lastIp);
      console.log(`Usuario desbloqueado: ${user.email}`);
    }
